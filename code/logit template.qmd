...
mutate(
  across(c(gender, smoke, bpmeds:diabetes), as.factor)
)
```
...
```{r uni}
uni_pregnant <- glm(p_diabetes ~ pregnant, 'binomial', df1)
summary(uni_pregnant)
exp(cbind(OR = coef(uni_pregnant), confint(uni_pregnant)))
broom::tidy(uni_pregnant, exponentiate = TRUE, conf.int = TRUE)

uni_glucose  <- glm(p_diabetes ~ glucose, family = 'binomial', data = df1)
summary(uni_glucose)
exp(cbind(OR = coef(uni_glucose), confint(uni_glucose)))
broom::tidy(uni_glucose, exponentiate = TRUE, conf.int = TRUE)

summary(glm(p_diabetes ~ glucose, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ pressure, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ triceps, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ insulin, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ mass, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ pedigree, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ age, family = 'binomial', data = df1))
summary(glm(p_diabetes ~ pedigree, family = 'binomial', data = df1))
